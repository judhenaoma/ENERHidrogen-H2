...
async function guardarRegistrosEolica50 ( valorVelocidadViento, lng, lat ){
    // Fecha actual
    ...
    const coeficienteMaquina = parseFloat(document.getElementById("coef_maquina").value)
    // Diametro del rotor
    const diametroRotor = parseFloat(document.getElementById("diametro_rotor").value) // DR
    // Factor de planta
    const factorPlanta = parseFloat(document.getElementById("factor_planta_eo").value)
    // Potencia eolica calculada
    const potenciaEolica =   document.getElementById("resultado_potencia_eo").innerHTML 
    // Generacion anual de energia Eólica
    const generacionAnualEolica =  document.getElementById("resultado_generacion_eo").innerHTML
    // Área disponible 
    const areaCapturada = parseFloat(document.getElementById("area_lote").value) // AreaLote
    const areaLoteHa =  areaCapturada !== '' ? areaCapturada : 'NA'
 
    // Numero de aerogeneradores
    const numeroAerogeneradores = areaCapturada !== '' ? document.getElementById("resultado_aerogen").innerHTML : 'NA'
    // Potencia parque eolico
    const potenciaParqueEolico = areaCapturada !== '' ? document.getElementById("resultado_pot_parque_eolico").innerHTML : 'NA'
    // Generación parque eolico
    const generacionParqueEolico = areaCapturada !== '' ? document.getElementById("resultado_gen_parque_eolico").innerHTML : 'NA'  
...
window.guardarRegistrosEolica50 = guardarRegistrosEolica50;
